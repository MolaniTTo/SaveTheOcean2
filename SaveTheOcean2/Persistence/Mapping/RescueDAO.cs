...
namespace AC4.Persistence.Mapping
{
    public class RescueDAO : IRescueDAO
    {
         ...
         {
              connection.Open();
              using (NpgsqlCommand command = new NpgsqlCommand("INSERT INTO Rescue (rescueid, Date, Location, GA, AnimalBreed, AnimalName, AnimalWeight, AnimalSuperfamily)VALUES(@IdRescue, @Date, @Location, @GA, @AnimalBreed, @AnimalName, @AnimalWeight, @AnimalSuperfamily);", connection))
              {
        ...
        public List<Rescue> GetRescueById(string id)
        {
            ...
            {
                connection.Open();
                using (NpgsqlCommand command = new NpgsqlCommand("SELECT * FROM Rescue WHERE rescueid = @IdRescue;", connection))
                {
